...
Monitoring orchestrator - wraps CloudWatch, Prometheus, AppSignals, ApplicationSignals MCP servers

**Port**: 9190 | **Status**: 🟢 Scaffolded | **Compliance**: ✅ Born Compliant

## Overview

Virons AI MCP server for monitoring operations. This server follows Domain-Driven Design (DDD) and Test-Driven Development (TDD) principles, with compliance built-in from day one.

## Quick Start

```bash
# Install dependencies
cd virons-monitoring-mcp-server
uv sync
...
# Run server
uv run virons-monitoring-mcp-server

# With write operations enabled
uv run virons-monitoring-mcp-server --allow-write
```
...
```bash
# Build
docker build -t virons-monitoring-mcp-server .

# Run
docker run -it virons-monitoring-mcp-server
```
...
Edit `virons/monitoring_mcp_server/server.py`:
...
**Generated by**: `virons-scaffold` | **Domain**: Server Domain | **Context**: Monitoring MCP Server
