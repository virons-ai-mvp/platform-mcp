...
# Deploy Infrastructure MCP to ECS

set -e

AWS_REGION="${AWS_REGION:-eu-central-1}"
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
ECR_REPO="virons-infrastructure-mcp"
IMAGE_TAG="${IMAGE_TAG:-latest}"

echo "🚀 Deploying Infrastructure MCP to ECS"
...
aws logs create-log-group --log-group-name /ecs/virons-infrastructure-mcp --region $AWS_REGION 2>/dev/null || true
