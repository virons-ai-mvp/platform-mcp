...
Security orchestrator - wraps CloudTrail, IAM, Well-Architected Security, Gitleaks, Compliance Gate MCP servers

**Port**: 9100 | **Status**: 🟢 Scaffolded | **Compliance**: ✅ Born Compliant

## Overview

Virons AI MCP server for security operations. This server follows Domain-Driven Design (DDD) and Test-Driven Development (TDD) principles, with compliance built-in from day one.

## Quick Start

```bash
# Install dependencies
cd virons-security-mcp-server
uv sync
...
# Run server
uv run virons-security-mcp-server

# With write operations enabled
uv run virons-security-mcp-server --allow-write
```
...
```bash
# Build
docker build -t virons-security-mcp-server .

# Run
docker run -it virons-security-mcp-server
```
...
Edit `virons/security_mcp_server/server.py`:
...
**Generated by**: `virons-scaffold` | **Domain**: Server Domain | **Context**: Security MCP Server
